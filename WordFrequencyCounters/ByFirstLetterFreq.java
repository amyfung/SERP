...
public class ByFirstLetterFreq{
	...
	List<Hashtable<String,Integer>> mainTable = Collections.synchronizedList(new ArrayList<Hashtable<String,Integer>>());

	ByFirstLetterFreq(){
		//create a Hashtable for every letter in the alphabet and store them
		//in the slots of ArrayList accordingly
		//For example: Hashtable for words starting with "A" will have index of 0 in the ArrayList
		for(int i=0 ; i < 26 ; i++){
	...
	public void  count(String filename) {
		...
		synchronized(mainTable) {
			while (sc.hasNext()) {
				...
				if(index >= 0 && index<26){
					Hashtable<String,Integer> letterMap = mainTable.get(index);
